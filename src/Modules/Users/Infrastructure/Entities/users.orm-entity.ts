...
export class Users_ORM_Entity {
  ...
  @JoinColumn({
    name: 'spv_id',
    foreignKeyConstraintName: 'FK_SpvID_at_UsersTableFieldRole',
  }) // ngarah ke kolom di database
  spv: Users_ORM_Entity;

  @OneToMany(
    () => ClientInternal_ORM_Entity,
    (clientInternal) => clientInternal.marketing,
  )
  clientInternals: ClientInternal_ORM_Entity[];
  @OneToMany(
    () => ClientExternal_ORM_Entity,
    (clientExternal) => clientExternal.marketing,
  )
  ...
  clientInternalProfiles: ClientInternalProfile_ORM_Entity[];
