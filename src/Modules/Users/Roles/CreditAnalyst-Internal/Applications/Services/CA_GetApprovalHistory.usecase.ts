...
export class CA_GetApprovalHistory_UseCase {
  ...
  async execute(page = 1, pageSize = 10, searchQuery = '') {
    try {
      ...
      const { data, total } =
        await this.loanAppRepo.callSP_CA_GetApprovalHistory_Internal(page, pageSize);
      ...
      const formattedData = filteredData.map((item) => ({
        ...
        nama_nasabah: item.nama_lengkap || '-', // sesuai field dari SP
      ...
      const message = err instanceof Error ? err.message : 'Gagal mengambil data pengajuan';
