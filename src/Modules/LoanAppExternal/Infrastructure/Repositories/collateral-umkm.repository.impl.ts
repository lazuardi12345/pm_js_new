...
{
  constructor(
    @InjectRepository(CollateralByUMKM_ORM_Entity)
    private readonly ormRepository: Repository<CollateralByUMKM_ORM_Entity>,
  ) {}

  // ========================== MAPPER: ORM → DOMAIN ==========================
  private toDomain(orm: CollateralByUMKM_ORM_Entity): CollateralByUMKM {
  ...
  private toOrmPartial(
    partial: Partial<CollateralByUMKM>,
  ): Partial<CollateralByUMKM_ORM_Entity> {
    const orm: Partial<CollateralByUMKM_ORM_Entity> = {};
    ...
    if (partial.created_at !== undefined)
      orm.created_at = partial.created_at ?? new Date();
    if (partial.updated_at !== undefined)
      orm.updated_at = partial.updated_at ?? new Date();
    if (partial.deleted_at !== undefined)
      orm.deleted_at = partial.deleted_at ?? null;
  ...
  async save(data: CollateralByUMKM): Promise<CollateralByUMKM> {
    ...
    return this.toDomain(saved as CollateralByUMKM_ORM_Entity);
  }

  async update(
    id: number,
    data: Partial<CollateralByUMKM>,
    ...
    if (!updated)
      throw new NotFoundException(`CollateralByUMKM with ID ${id} not found`);
